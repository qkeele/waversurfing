...
class SpotService: ObservableObject {
    ...
    func getSpot(byId id: UUID) async throws -> Spot {
    ...
    func getSpotByReportId(reportID: UUID) async throws -> Spot? {
        // Step 1: Get spot_id from reports table
        let reportResponse = try await client
            .from("reports")
            .select("spot_id")
        ...
        let spot = try JSONDecoder().decode(Spot.self, from: spotResponse.data)
        return spot
    }
